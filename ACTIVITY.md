...
## 2026-07-24
🎧 **新 lesson：[Notting Hill — 假扮《馬與獵犬》記者的完美掩護](lessons/2026-07-24-notting-hill-horse-and-hound.md)**（dialogue · B2 · RealLife English app · 講師 Ethan · ~4 min）——Cian 指著 RealLife app 視窗說「收錄目前該部影片的 transcript 製作一個 lesson」，用 computer-use 讀 app 內建 transcript ＋ Vocabulary 卡片取得課程內容（app 課名「Horse & HOUND Magazine — A Perfect Cover for William!」）。素材＝《Notting Hill》(1999) 麗池飯店媒體聯訪場景：書店老闆 William 被公關誤認成記者，硬掰自己來自真實存在的英國馬術雜誌《Horse & Hound》，於是要對一部**設定在外太空**的科幻片問出跟馬有關的問題。**外部事實查證**（照 Soul 那課的前例）：台詞講者歸屬經**兩個獨立來源交叉查證**（WebSearch ＋ dailyscript 劇本）確認——「You've got five minutes.」與「你是 Horse & Hound 的？」出自公關 **Karen**（非 Anna）、「Is that so?」才是 **Anna**、道歉與 fire away 兩句是 **William**。判 dialogue track；per-lesson `speaker_voices`：William＝en-GB 男聲 −6% 語速（Hugh Grant 式支吾）、Anna＝en-US 女聲、Karen＝en-GB 女聲。模組：正文（短台詞雙語順流）、Key Vocab 6 字、**Sentence Anatomy 兩則**——`I was wondering whether...`（委婉開口句，⚠️**間接問句不倒裝**，正中 Cian 中式直譯弱點）＋英式 `..., shall I?` 徵詢句尾、**Connected Speech（英式 non-rhotic linking R：`fire away` → fai-uh-**r**-away）**、Cultural Notes（Horse & Hound 真有其刊／hound 雙義呼應狗仔追明星的全片主題／ring vs call 英美分野／press junket／Hugh Grant 招牌填充詞）、Study Tips。新增 **6 keeper**：**fire away**（儘管問吧，⚠️不及物不可接受詞）／**ring someone back**（回電，英式，⚠️可拆 ring you back ✅）／**hound**（獵犬＋緊追不捨騷擾，一字兩義同一畫面）／**put someone on the spot**（當場將軍，⚠️the 不可省）／**right off the bat**（劈頭第一件事，棒球語源）／**come over someone**（情緒突然襲來，招牌句 I don't know what came over me，⚠️過去式 came）。全 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 281→**287**。
📚 **單字批次練習：dismayed / utterly / colonial power / in a negative light / fester**——Cian 連續丟五個字問意思，批次存檔。**第一批套用新雙軸標記**（卡片與檔案都標常見度＋CEFR）。新增 **dismayed**（C1·低頻，錯愕＝失望＋驚訝的綜合體，`be dismayed by/at/to/that`、句首 `to my dismay`；⚠️只能被動，`I dismayed` 是錯的）、**utterly**（C1·中頻，比 very 高一階的加強副詞；⚠️**只配極端／負面形容詞**——`utterly useless/exhausted/ridiculous` ✅ 但 ~~utterly good/tired~~ ❌，且不修飾動詞；⚠️別跟 `utter` adj./v. 混）、**colonial power**（B2·低頻，`power`＝強權國家非力量，帶出 superpower／great power／rising power 整排；⚠️**重音搬家** **col**ony vs co**lo**nial）、**in a negative light**（B2·中頻，`light`＝看待角度；⚠️**修掉 Cian 的搭配錯誤**——他寫 `project in a negative light`，但 light 配 **cast**／portray／paint／put，`project` 配的是 **image**；面試講前東家的高頻語境）、**fester**（C1·低頻，因**被忽略**而在暗處腐爛，招牌句型 `let sth fester`；⚠️**不及物**，`He festered the problem` 是錯的；對比 escalate／deteriorate／linger）。全 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 276→**281**。
▶️ **單字雙軸分級：全庫 backfill `cefr` + `frequency` ＋ app 顯示**——Cian 要求「未來單字問答與紀錄同時標**是否常見**、**坐落哪個級別**」。級別標準先讓 Cian 裁決（CEFR／台灣 7000 字／詞頻 band／GEPT 四選一）→ 選 **CEFR**，理由是 repo 已有 lesson `level` 在用，不再多立一把尺。**規則**寫進 [`reference/cefr-leveling-process.md` §8](reference/cefr-leveling-process.md)（SOP 由「Lesson 分級」擴編為涵蓋單字，v1→v2）：`cefr`＝要多好的程度才會用、`frequency`＝實際多常遇到，**兩條軸獨立**，四象限都有真實例子（`alpaca` A2+低頻＝簡單但用不到；`acknowledge` B2+高頻＝難但值得投資），交叉後才決定一個字練到「主動會用」或「被動認得」；另定六個錨點字、`?` 不確定標記、多義字看主要學習義、片語照整體判。**Backfill**：276 字由**單一 assessor agent 在同一 context** 一次評完（照 §5B「多個 agent 各評各的會飄」），輸出 JSON 後由**確定性腳本**套用（不讓 agent 直接改 276 個檔）；我方獨立驗證＝276/276 對得上、值全合法、每個 slug 對得到實體檔、錨點字全部落回定義位置。分佈 A2 22 · B1 70 · B2 156 · C1 26 · C2 2，常見度 25/141/110，**41 字帶 `?`**（多為 AGI-pilled／dogfood 這類 CEFR 本來就不適用的新造詞與 slang）。⚠️ assessor 主動回報兩個判斷張力：`baby` 在 index 的釋義是「媽寶」貶義故評 B1? 而非 A1 錨點義；`ship`／`lemon`／`man` 等 7 字評的是**冷門義**而非本字頻率——兩者都留待日後決定要不要翻。**App**：`content.config.ts` 加兩欄（`cefr` 在 schema 層 `.transform()` 成 `{level, hedged}`，消費端不可能忘了處理 `?`）、沿用既有 `LevelBadge` 加 `hedged` prop、新 `FrequencyBadge`（**刻意不給專屬顏色**——`Layout.astro` 色彩規範白紙黑字寫「CEFR 色階是唯一的多色例外」；初版用點數 3/2/1 顆表達 tier，**同日經 Cian 指正簡化為純文字、不加粗**，理由是常見度屬輔助資訊不該跟難度搶視覺）、單字卡另有「難度／常見度」兩行白話。**驗證**：`astro check` 0 error；live 276/276 兩 badge 全上、41 hedged、各 tier 標籤正確；深色＋手機版實測；點字朗讀 mp3 回 **206** 未受影響；**frontmatter 改動零汙染音檔**——manifest **21 筆新增、0 筆修改**（那 21 筆是上一棒 solid/pace/partway/work on it 遺留未生成的，非本次造成）。`/simplify` 修 10 項（`z.enum(FREQUENCY_TIERS)`、CEFR 文法由 `CEFR_LEVELS` 組出讓 schema 與 parser 共用、`CEFR_HEDGE_NOTE` 單一來源、刪 6 個違反設計系統的寫死色碼、dots 一度收斂成 `--dot` 變數（後續整段隨簡化移除）、合併重複 CSS、刪死 class）；跳過全域 `.pill` 底盤（要動 5 個既有呼叫點，超出 diff 範圍）。⚠️ 4 個 review agent 只有 reuse 跑完整，另 3 個因**沒照指示只回報、直接編輯檔案**與我的編輯互相覆蓋而被中止。詞庫維持 **276**。
📚 **單字批次練習：solid / pace / partway / work on it ＋複習 canonical**——Cian 連續丟自撰句子與單字問意思，採批次存檔模式（卡片即時給、持久化延到「一起存」才一次做）。新增 **solid**（穩固可靠、沒問題，⚠️多義字：物理固體 vs 口語「沒問題」）、**pace**（步調/速度，招牌搭配 at your own pace／keep pace with／pick up the pace）、**partway**（走到一半，`partway through + N`，對比 halfway 強調剛好一半）、**work on it**（處理、著手做／努力改善，`work on + N` 高頻片語動詞）。**複習 canonical**（★1 rc1→2，Cian 再次問起、正確回想出 canonical URL／canonical example 用法）。全新字 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 272→**276**。

## 2026-07-23
▶️ **App：MARGINALIA 寬螢幕書卷版面（pilot 只上 Forget Me Not 這一篇）**——接續上一棒的雙語兩欄閱讀，把大螢幕版面做成「書卷＋活頁邊」：≥1024px 每個 `## section` 變 `[ rail 236px | well ≤64ch | gutter ]` 書框、全出血分頁線＋✿、folio（03/08）＋章節標籤在 rail；輔助教材搬進右側 margin rail——TOEIC 關鍵字 chips、測驗答案（可折疊）、Sentence Anatomy 要點變編號邊註、Coach Max 署名邊註；表格與 Close Reading 兩軌橫跨整版；窄螢幕退回「內容優先」單欄堆疊、答案仍可折疊不洩題。流程：**先問 Cian 裁決 Full vs Lite → 選 Full**（＝核准的 mockup）；**計畫先過 codex 共識 review（2 輪 APPROVE）**——codex R1 抓到 4 條 BLOCKING（我 Explore agent 漏掉的真 bug）：① rail 若用 blockquote 會被 `generate-audio` 的獨立 raw-markdown 解析判 speakable → 改用**非-blockquote `[!type]` marker**（`collectSpeakable` 只走 blockquote／word 表）；② 改 `Layout.astro` 全域 CSS 會換掉 shared `Layout.<hash>.css` → 其餘每頁 `<link>` 全變 → 改由 **plugin 把 `marginalia.css` inline 注入該篇 content**、零碰任何 shared 檔；③ rail 排在 well 之前會在窄螢幕先洩答案 → DOM 改**來源順序**＋答案改**可折疊 `<details>`**；④ `.m-anno` 編號改 CSS counter。落地：新 `app/src/rehype/inject-marginalia.mjs`（gate `wide_layout: marginalia`、**relocation-only**、註冊在最後、readFileSync 一次注入 CSS）＋ `app/src/styles/marginalia.css`。**其餘 24 課＋全部 vocab byte-for-byte 不變**（baseline 前後對全 dist HTML SHA-256 逐檔比對＝只有 pilot 那頁變）、**零音檔重生**（`generated=0`、mp3／manifest／sidecar SHA 全等）、`astro check` 0 error。live 數值驗：main 1010、rail 236／well 鎖 64ch、3 塊全版（2 表＋bilingual 962px）、7 條編號 anno、4 個折疊答案（0 預設展開）、窄螢幕內容先於答案且 folio 隱藏；Close Reading 兩軌點播真 mp3 206＋karaoke sidecar 200（未動 speech.ts）。`/simplify` 4-agent（去掉死的 `chip` marker、chips 折掉一次 `hastText` walk；pilot HTML byte 不變＝純重構）。收工刪 `_preview/layouts.html` 與 HANDOFF。詳見 [`lessons/_conventions.md` §5.7](lessons/_conventions.md)。
📚 **新增單字：role**——Cian 問面試常見開場句 "What are you looking for in your next role?" 裡 role 的意思。重點是 `role` 是「職位/職務」的正式說法（比 job 文雅、面試履歷常用），招牌句型 `in your next role`；⚠️ 同音字 `roll`（滾動）別搞混。★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 271→**272**。
