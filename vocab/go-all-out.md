---
word: go all out
phonetic: /ɡoʊ ɔːl aʊt/
pos: phr.v
zh: 卯足全力；火力全開
cefr: "B2"
frequency: mid
proficiency: 2
first_seen: 2026-04-27
last_reviewed: 2026-05-02
review_count: 4
tags: [idiom, effort, performance]
source: "RealLife App: Tom Holland × Gordon Ramsay"
---

## Examples
> "Tom surprised everyone by going all out with fishnets, a wig, and incredible choreography."

> Tom 卯足全力穿漁網襪、戴假髮、配上瘋狂編舞，讓所有人嚇一跳。

> "We went all out for her birthday party."

> 我們為她的生日派對火力全開。

## Usage tip
- 常見句型：**go all out for [event]** / **go all out on [thing]**
- 帶有「不保留任何餘力、玩很大」的情緒色彩
- 與 "take it easy"（放輕鬆）正好相反

## Synonyms
give it your all, pull out all the stops, leave nothing on the table

## Encounters
- 2026-04-27 — first learned (RealLife App: Tom Holland × Gordon Ramsay)
- 2026-04-28 — used in writing drill: "My sister is going all out for her wedding..." — 主結構抓對，但卡在 `for holding a wedding`（應為 `for + N`）和 3rd person sg `go → is going`
- 2026-04-30 — quiz miss: 在 Tom Holland 情境誤選 "poured into"。釐清：`pour into` 需要 object（"pour his heart into"），單獨不能填空；`go all out for/on` 才是「卯足全力做某事」的標準片語。降級警示但仍維持 ★3
- 2026-05-02 — quiz miss ❌：誤答 "go all in"。釐清：**go all in** = 全押／梭哈（賭博、承諾把資源全投入）；**go all out** = 卯足全力做某事（強調 effort 程度）。連續第二次 miss → **降 ★3 → ★2**。記法：all OUT = 力氣全掏 OUT 出來
