---
word: costume
phonetic: /ˈkɑːstuːm/
pos: n
zh: 戲服；演出服；變裝服
proficiency: 3
first_seen: 2026-04-27
last_reviewed: 2026-04-27
review_count: 1
tags: [fashion, performance, easily-confused]
source: "RealLife App: Tom Holland × Gordon Ramsay"
---

## Examples
> "Her Halloween costume was a giant taco."
> 她的萬聖節服裝是一個大塔可餅。

> "Tom is talking about different costume items he decided to wear."
> Tom 在講他決定穿的不同戲服配件。

## Usage tip
- ⚠️ 別跟 **custom** /ˈkʌstəm/ 混淆 — 拼字差一個字母，意思天差地別：
  - **costume** = 戲服
  - **custom** = 習俗 / 客製化
- 常見搭配：Halloween costume, period costume（古裝）, costume design, costume party
- **costumes** 也指劇組裡的「服裝部門」

## Synonyms
outfit, garb, attire (formal), getup (informal)

## Encounters
- 2026-04-27 — first learned (RealLife App: Tom Holland × Gordon Ramsay)
