---
word: on the chopping block
phonetic: /ɑːn ðə ˈtʃɑːpɪŋ blɑːk/
pos: idiom
zh: 面臨被砍；可能遭解雇或裁撤
proficiency: 1
first_seen: 2026-06-23
last_reviewed: 2026-06-25
review_count: 2
tags: [idiom, work, layoffs, risk, vivid-language]
source: "lesson: The Devil Wears Prada first day"
---

## Examples
> "Several projects are on the chopping block after the budget cut."

> 預算削減後，有幾個專案面臨被砍。

## Usage tip
畫面來自砧板上即將被切的東西，因此比 `on the line` 更接近「已被列為裁撤對象」。人可指可能被解雇；預算、部門或專案則指可能被取消。

## Encounters
- 2026-06-23 — first learned (RealLife English: "My head is on the chopping block.")
- 2026-06-25 — reviewed (Coach Max 拆解砧板畫面，並對比 `on the line` vs `on the chopping block`)
