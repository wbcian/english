...
## 快照（隨更新同步）
- 已收錄單字：287（**全部標了 `cefr` + `frequency` 雙軸**）
- 5★ 精熟：0　 3–4★ 學習中：8　 1–2★ 新／脆弱：279
- CEFR 分佈：A2 22 · B1 71 · B2 163 · C1 29 · C2 2　｜　常見度：高頻 25 · 中頻 147 · 低頻 115　｜　41 字帶 `?`（估計/邊界）
- 最近活動日期：2026-07-24（🎧 **新 lesson：[Notting Hill — 假扮《馬與獵犬》記者的完美掩護](lessons/2026-07-24-notting-hill-horse-and-hound.md)**（dialogue · B2 · RealLife English app · 講師 Ethan · ~4 min）——Cian 指著 RealLife app 視窗要求「收錄目前該部影片的 transcript 製作一個 lesson」，用 computer-use 讀 app 內建 transcript ＋ Vocabulary 卡片取材（app 課名「Horse & HOUND Magazine — A Perfect Cover for William!」）。素材＝《Notting Hill》(1999) 麗池飯店媒體聯訪：書店老闆 William 被公關誤認成記者，硬掰來自真實英國馬術雜誌《Horse & Hound》，於是得對一部**設定在外太空**的科幻片問馬的問題。**講者歸屬經兩獨立來源交叉查證**（WebSearch ＋ dailyscript 劇本）：「You've got five minutes.」與「你是 Horse & Hound 的？」出自公關 **Karen**（非 Anna）、「Is that so?」才是 **Anna**。judged dialogue track；per-lesson `speaker_voices` 三人分聲（William＝en-GB 男 −6% 語速）。模組：正文短台詞雙語順流、Key Vocab 6 字、**Sentence Anatomy**（`I was wondering whether...` ⚠️**間接問句不倒裝**、英式 `..., shall I?`）、**Connected Speech（英式 non-rhotic linking R：fire away）**、Cultural Notes、Study Tips。新增 **6 keeper**：**fire away**／**ring someone back**（英式，⚠️可拆）／**hound**（獵犬＋緊追騷擾，雙義呼應狗仔追明星的全片主題）／**put someone on the spot**／**right off the bat**／**come over someone**。全 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 281→**287**）
- 上一活動日期：2026-07-24（📚 **單字批次練習：dismayed / utterly / colonial power / in a negative light / fester**——第一批套用新雙軸標記。⚠️ 修掉 Cian 的搭配錯誤 `project in a negative light`（light 配 cast／portray，`project` 配 image）；另兩個文法陷阱：`dismayed` 只能被動、`fester` 不及物。詞庫 276→**281**）
- 上一活動日期：2026-07-24（▶️ **單字雙軸分級：全庫 backfill `cefr` + `frequency` ＋ app 顯示**——Cian 要求「單字問答與紀錄同時標常見度與級別」，選定 **CEFR** 當級別標準（與 lesson `level` 同一把尺，不另立標準）。定義寫進 [`reference/cefr-leveling-process.md` §8](reference/cefr-leveling-process.md)：`cefr`（要多好才會用）與 `frequency`（實際多常遇到）是**兩條獨立的軸**，四象限都存在（`alpaca` A2+低頻 vs `acknowledge` B2+高頻），交叉後才決定一個字要練到「主動會用」還是「看懂就好」；判斷型估計故沒把握一律標 `?`。既有 276 字由**單一 assessor agent 在同一 context** 一次評完（照 §5B「同一把尺才不會飄」），錨點字全部落回定義位置（`role`→B1、`acknowledge`→B2、`incorrigible`→C2）；套用後 SSOT ↔ `_index` parity 0 mismatch。app 端：清單與單字卡都顯示兩個 badge（CEFR 沿用既有 `LevelBadge` 色階、常見度**純文字不加粗**——初版做成點數 badge，同日經 Cian 指正改為樸素文字，因為常見度是輔助資訊、不該跟難度搶視覺），單字卡另有「難度／常見度」兩行白話說明。`?` 照實顯示不抹掉。frontmatter 改動**未汙染任何音檔 hash**（manifest 21 筆新增、**0 筆修改**）。`/simplify` 修 10 項（schema `.transform` 讓解析只做一次、CEFR 文法與 hedge 說明各收斂成單一來源、刪 6 個違反設計系統的色碼）。詞庫維持 **276**）
...
- 上一活動日期：2026-07-23（▶️ **App：雙語兩欄閱讀 UI（pilot 只上 Forget Me Not 這一篇）**——需求＝中英分兩軌並排、**點整句就播**（不用瞄 icon）、hover 顯示對應句、保留 karaoke 蔓延。流程照 Cian 指定「先討論＋預覽確認才動 app」：**3 人設計 agent team**（UX／版面／實作）平行討論收斂 → **真音檔＋真 karaoke 的互動 mockup** 給 Cian 試玩 → 裁決兩點：**只這篇先試**＋**目錄一律保留**。落地：新 `app/src/rehype/inject-bilingual.mjs`（**純搬移** rehype、gate by frontmatter `two_lane:true`；只把成對 `<blockquote>` 併成 `.bilingual` grid、**不碰 `<p>`/`.w`/文字 → 音檔 hash＋karaoke byte 不變、零重生**）＋ `speech.ts`（整段點擊播放、點中文播對應英文、hover 雙向 `.lit`＋播放鏡射 `.mate-speaking`、mate 用 WeakMap wire 時快取 O(1)；▶ 移句首左 rail 並淡化＝保留給鍵盤/a11y）＋ Layout.astro 兩欄 CSS（≥1024px、**交錯 grid** 讓窄螢幕仍「英-中-英-中」、寬螢幕**只往右撐開**固定讓開左側 spine 26px）。**其餘 24 課＋vocab 全 byte 不變**（gate 驗 Soul/vocab `.bilingual`=0）。`/simplify` 4-agent 收斂（`hasActiveSelection` 去重、EN wiring 移出通用迴圈與 ZH 合併成 `wireBilingualSentence`、mate WeakMap、plugin 去重掃描）。astro build 綠 generated=0、live 數值全驗（真 mp3 206、289 span 全對、右緣讓開目錄）。詳見 [ACTIVITY.md](ACTIVITY.md)。Day 2 後可評估推廣其他 reading 課）
- 上一活動日期：2026-07-23（🎧 **新 lesson：[Forget Me Not — 妥瑞症女孩的漂泊與救贖](lessons/2026-07-23-forget-me-not-brings-tourette-syndrome-to-light.md)**（reading · B2 · English Digest 實用空中美語 Unit 15-1 文學單元 · 本地 whisper 轉錄 ~32 min）：Cian 貼 Apple Podcasts 連結。**解析器卡 ambiguous**——同節目同日上架 Unit 15-1／15-2 標題幾乎一樣、模糊比對 best−runner < 0.12 分不出，但連結本身標明 15-1 → 用候選第一筆 mp3 繞過 resolver、沿用同一套轉錄函式庫（先 HTTP 206 探測確認 `rssFileVip.mp3` 非付費牆）。節目雙師制：Tom & Stephanie 英解＋Patrick 老師中解，把 Ellie Terry 處女作小說《Forget Me Not》（中譯《月亮都是一樣的》）**整本劇情分兩幕摘要朗讀**（Act 1 身世＋妥瑞症＋第一天霸凌；Act 2 學生會長靜松、友誼萌芽、衝突停學、衝動搬家、路上和解＋新校坦白）。判 **reading track**（雜誌閱讀導讀、audio: true），課文兩幕全文收錄當 Close Reading。模組：Article Map（兩幕骨架表）、Close Reading（雙語 blockquote 順流）、Key Vocabulary（20 字表）、Reading Comprehension（**TOEIC Part 7** 五題＋details 折疊答案）、Sentence Anatomy（**分詞構句** `Drawn to Callie, ...` 正中 Cian 懸垂分詞弱點＋擬人關係子句 `a secret that refuses to stay hidden`，兩句 Patrick 老師節目裡也親自拆過）、Cultural Notes（勿忘我花語雙關／military brat／student body president／physician vs doctor／debut 尾 t 不發音／Las Vegas）、Study Tips。新增 **12 keeper**：**rooted**（有歸屬感，反義 uprooted）／**relocation**（搬遷）／**conceal**（隱藏，⚠️與 disclose 成對）／**physician**（醫師，vs doctor）／**prominent**（顯眼傑出）／**disclose**（揭露）／**debut**（處女作，⚠️尾 t 不發音）／**dread**（畏懼，⚠️後接 V-ing）／**mock**（嘲弄／模擬考）／**erupt**（情緒／火山爆發）／**impulsively**（衝動地）／**suspend**（停學／停職）。**複習 3 舊字**：**manifest**（顯現義再現，★1 rc2→3）、**reputation**（★1 rc1→2）、**confront**（★1 rc1→2）。全新字 ★1，雙語 karaoke 例句＋ `_index` 同步。raw transcript `transcripts/2026-07-23-forget-me-not-brings-tourette-syndrome-to-light.txt`。詞庫 257→**269**。系列 `forget-me-not`，Day 2 待出）
- 上一活動日期：2026-07-21（🎧 **新 lesson：面試自我介紹拆解**（talk · B2 · **🔒 私人、未列出、不進 repo，僅本機**）：拿 Cian 自撰英文口說稿當正文逐字檢查。總評**結構 A、時態 A−、用字 B+**——命中英文面試三段式（Now → Path → Why you），四種時態全部切對。要修四類：⚠️ **懸垂分詞**（句尾 `, V-ing` 隱藏主詞對不上主句，全篇唯一真文法錯，附三修法＋自檢法）、重複用字、平行結構鬆掉、中式搭配三連（technical practice／matched…well／deliver features）。模組：Sentence Anatomy 六段、Polish Pass（八處對照＋可背全文）、Useful Phrases 十句骨架、Connected Speech、Cultural Notes。新增 6 keeper：**come to a close**／**growth phase**／**line up with**／**have a personal connection to**／**ship**／**access point**。**Cian 自己改出 V2**：八條問題自主修掉五條（含採用 `ship`），⚠️懸垂分詞未修、新長出 `also`／`really` ×3 → Coach Max 給 **V3＝動四刀**，lesson 加 `V2 修訂對照` 一節。用法問答帶出 3 keeper：**wrap up**／**be there through**／**around（大約梯度）**；⭐ **wrap up 學會當天即主動調用**（review_count 1→2）。詞庫 248→**257**。🔒 同時上線 **`unlisted` frontmatter 欄位**：標 `unlisted: true` → 首頁列表／搜尋／計數皆跳過、網址仍可直接開；⚠️ unlisted ≠ private，真私密內容另走 `.gitignore`）
- 上一活動日期：2026-07-14（📚 **中翻英辨析＋新增單字：「素材」怎麼講**——Cian 問「素材」的英文，拆成四個依情境分工的字：**material**（泛用）、**footage**（限影像素材）、**content**（行銷/社群語境，⚠️重音陷阱）、**source material**（改編作品的原著）。另 📚 **新增單字：merchant** /ˈmɜːrtʃənt/（n.，商人、貿易商）——語感偏傳統/大宗貿易商，現代延伸見於電商 online merchant、merchant account。全 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 243→**248**）
- 上一活動日期：2026-07-12（📚 **新增單字：capstone** /ˈkæpstoʊn/（n.，壓頂石；比喻巔峰之作、集大成的收尾）——Cian 傳單字想學。重點：比喻義在**「完成、收尾」而非只是「最高」**（vs pinnacle/peak）；招牌搭配 **capstone project/course**（美國大學畢業專題高頻用語）＋句型 `the capstone of + 一段歷程`；配對記憶 **cornerstone**（基石，底部起頭）vs capstone（頂端收尾）。★1，雙語 karaoke 例句兩句＋ `_index` 同步。詞庫 242→**243**）
...
- 上一活動日期：2026-07-03（🎧 **新 lesson：Soul — 追夢、脆弱，與「你到底跑哪去了？」**（dialogue · B1 · RealLife English · 講師 Ethan 用 Pixar《Soul》Joe Gardner 試音場景 · ~4 min）：Cian 傳字幕截圖，設計出「先完整播一次 Dorothea↔Joe 對白場景（split TTS 聲音）、再進 Ethan 教學拆解」的結構。**間隔複習亮點**：今天稍早才學會的 `zone out`／`in the zone`／`kill it`（皆 ★1）第一次落在真實電影場景複習（review_count 1→2）。**外部事實查證修正**（GATE：獨立查證原片字幕/腳本來源後修正設計稿假設）：原設計稿把「Uh, sorry, I zoned out a little back there」整句歸給 Dorothea，經三個獨立來源交叉查證（scrapsfromtheloft.com 逐字稿 fetch＋兩輪 WebSearch）後確認**這句其實是 Joe 自己承認恍神**、「Joe Gardner, where have you been?」才是 Dorothea 的提問——已在 lesson 內用真實版本並附修正說明。新增 5 個 keeper：**soundcheck**（試音）、**full-time position**（全職職位）、**Teach**（稱呼老師的俚語）、**where have you been**（對埋沒才華表驚訝的慣用句）、**take the wheel**（接手掌控）。模組含 Connected Speech（美式 T-flap）、Sentence Anatomy（"Where have you been?" 慣用現在完成式）。全新字 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 227→**232**）
- 上一活動日期：2026-07-03（📚 **單字批次練習（第二輪）：flow state + in the zone**——Cian 問「心流狀態怎麼說」。新增 **flow state**（心理學正式用語，源自 Csikszentmihalyi）與口語版 **be in the zone**（運動/電玩/日常閒聊常用）。教學亮點：`in the zone` 跟同日稍早學的 `zone out` 用同一個字 zone、意思完全相反（out＝恍神脫離 vs. in＝深陷專注），順勢做跨字對照複習。全 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 225→**227**）
- 上一活動日期：2026-07-03（📚 **單字批次練習：一次 7 字/片語（曖昧系 + 潮流黑話 + 職場表達）**——Cian 連續丟字，採批次存檔模式（卡片即時給、持久化延到「練習結束」才一次做）。新增 **be interested in someone**（曖昧初期含蓄有好感，例句含「偷瞄」情境 steal glances at）、**be into someone**（比 interested 更熱烈上頭）、**pick up**（一字四義：接人/自然學會/搭訕/接電話，⚠️可拆 pick you up ✅／pick up you ❌）、**swag**（自信氣場 vs 展會贈品雙義，⚠️稍過氣俚語）、**conversational in**（語言能力階梯 basic<conversational<fluent<native，履歷用語）、**zone out**（放空恍神，不及物不可拆）、**kill it**（誇獎「表現超好」，⚠️另義「關掉它」靠語境分，近義 nail it）。全 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 218→**225**）
- 上二活動日期：2026-07-02（📚 **新增單字：harvest** /ˈhɑːrvɪst/（n./v.，收穫、收成；收割、採收）——Cian 傳單字想學（Coach Max 一度誤當成「去收割一波單字」的指令）。重點：**名詞動詞同形**（the harvest／to harvest X）＋比喻用法 **harvest data／harvest energy**（大量蒐集），搭配 a bumper harvest、reap the harvest。★1，雙語 karaoke 例句兩句＋ `_index` 同步。詞庫 217→**218**）
...
- 最近 lesson：**[Notting Hill — 假扮《馬與獵犬》記者的完美掩護](lessons/2026-07-24-notting-hill-horse-and-hound.md)**（dialogue · B2 · RealLife English app · 講師 Ethan · ~4 min）— 素材取自 app 內建 transcript；《Notting Hill》麗池飯店媒體聯訪場景，William 冒充《Horse & Hound》記者訪問 Anna。正文（短台詞雙語順流）→ Key Vocab 6 字 → Sentence Anatomy（`I was wondering whether...` **間接問句不倒裝**＋英式 `..., shall I?`）→ Useful Phrases → Connected Speech（英式 linking R）→ Cultural Notes → Study Tips；新增 fire away / ring someone back / hound / put someone on the spot / right off the bat / come over someone
- 上一份 lesson：**[Forget Me Not — 妥瑞症女孩的漂泊與救贖](lessons/2026-07-23-forget-me-not-brings-tourette-syndrome-to-light.md)**（reading · B2 · English Digest 實用空中美語 Unit 15-1 文學單元 · Tom & Stephanie 英解＋Patrick 老師中解 · 本地 whisper 轉錄 ~32 min）— 雙師制節目把 Ellie Terry 處女作小說《Forget Me Not》（中譯《月亮都是一樣的》）整本劇情分兩幕摘要朗讀；判 reading track、課文兩幕全文收錄當 Close Reading。Article Map → Close Reading（雙語順流）→ Key Vocab 20 字 → Reading Comprehension（TOEIC Part 7 五題）→ Sentence Anatomy（**分詞構句** `Drawn to Callie,...` 正中懸垂分詞弱點＋擬人關係子句 `a secret that refuses to stay hidden`）→ Cultural Notes → Study Tips；新增 12 keeper rooted / relocation / conceal / physician / prominent / disclose / debut / dread / mock / erupt / impulsively / suspend，複習 manifest / reputation / confront
- 上一份 lesson：**面試自我介紹拆解**（talk · B2 · **🔒 私人：`unlisted: true` ＋ `.gitignore`，只在本機、repo 內查無此檔**）— 首份「以 Cian 自己的英文當正文」的 lesson：原稿跟讀 → Sentence Anatomy 六段（時態四層地圖／分詞構句／同位語／`where`／**懸垂分詞**／`make + O + adj`）→ Polish Pass 八處對照＋可背潤稿全文 → 自介骨架十句 → Connected Speech → 面試潛規則；新增 come to a close / growth phase / line up with / have a personal connection to / ship / access point
- 上一份 lesson：**[Soul — 追夢、脆弱，與「你到底跑哪去了？」](lessons/2026-07-03-soul-pursue-your-dream.md)**（dialogue · B1 · RealLife English · 講師 Ethan 用 Pixar《Soul》Joe Gardner 試音場景）— 先完整播一次 Dorothea↔Joe 對白場景（split TTS 聲音），再進 Ethan 教學拆解；複習今日稍早學的 zone out / in the zone / kill it，新增 soundcheck / full-time position / Teach / where have you been / take the wheel；Connected Speech 對準美式 T-flap，Sentence Anatomy 對準 "Where have you been?" 慣用現在完成式
- 上一份 lesson：**[好熱啊 P1：到底有多熱 + 冷氣救命](lessons/2026-07-01-ssyingwen-summer-heat-part-1-how-hot.md)**（talk · B1 · 時事英文 ssyingwen P04 · 三部曲 Part 1）— 用 15 首夏日金曲教夏天英文：hot 以外的說法、scorching/sweltering/sizzling 細分、熱浪與高溫警報、冷氣操作句；三部曲完整——[Part 2 濕熱/流汗/冰品/融化](lessons/2026-07-01-ssyingwen-summer-heat-part-2-sweat-and-ice.md)、[Part 3 防曬/海灘/夏至/午夜太陽](lessons/2026-07-01-ssyingwen-summer-heat-part-3-sun-and-beach.md)
- 上二份 lesson：**[The Devil Wears Prada — Miranda 的地獄級第一天](lessons/2026-06-23-the-devil-wears-prada-first-day.md)**（dialogue · B2 · RealLife English / Ksenia 字幕截圖）— 咖啡訂單、辦公室值守、跑腿分工與工作風險；11 個新表達 skimmed milk / drip coffee / run errands / deal with it / fling / foam / man / searing / chained to / on the line / on the chopping block；Sentence Anatomy 對準 `be chained to` 被動結構與 `if + 現在式` 後果句
...
- [`reference/learning-efficiency-research.md`](reference/learning-efficiency-research.md) — **學習科學 survey（2026-07-02 彙整複查版）**：英文怎麼學最有效率 / 一天多少 / 碎片化微學習可行嗎。最強結論＝間隔練習（spacing）> 集中猛 K；沒有跨技能「每分鐘效率排行榜」。碎片時間適合提取、輸入、小型形式聚焦與短輸出，整合口說／深度寫作需保護完整時段。**app roadmap P13 的依據**
