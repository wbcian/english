...
## 快照（隨更新同步）
- 已收錄單字：227
- 5★ 精熟：0　 3–4★ 學習中：8　 1–2★ 新／脆弱：219
- 最近活動日期：2026-07-03（📚 **單字批次練習（第二輪）：flow state + in the zone**——Cian 問「心流狀態怎麼說」。新增 **flow state**（心理學正式用語，源自 Csikszentmihalyi）與口語版 **be in the zone**（運動/電玩/日常閒聊常用）。教學亮點：`in the zone` 跟同日稍早學的 `zone out` 用同一個字 zone、意思完全相反（out＝恍神脫離 vs. in＝深陷專注），順勢做跨字對照複習。全 ★1，雙語 karaoke 例句＋ `_index` 同步。詞庫 225→**227**）
