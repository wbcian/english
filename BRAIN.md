...
## 快照（隨更新同步）
- 已收錄單字：254
- 5★ 精熟：0　 3–4★ 學習中：8　 1–2★ 新／脆弱：246
- 最近活動日期：2026-07-21（🎧 **新 lesson：面試自我介紹拆解**（talk · B2 · **🔒 私人、未列出、不進 repo，僅本機**）：拿 Cian 自撰英文口說稿當正文逐字檢查。總評**結構 A、時態 A−、用字 B+**——命中英文面試三段式（Now → Path → Why you），四種時態全部切對。要修四類：⚠️ **懸垂分詞**（句尾 `, V-ing` 隱藏主詞對不上主句，全篇唯一真文法錯，附三修法＋自檢法）、重複用字、平行結構鬆掉、中式搭配三連（technical practice／matched…well／deliver features）。模組：Sentence Anatomy 六段、Polish Pass（八處對照＋可背全文）、Useful Phrases 十句骨架、Connected Speech、Cultural Notes。新增 6 keeper：**come to a close**／**growth phase**／**line up with**／**have a personal connection to**／**ship**／**access point**。詞庫 248→**254**。🔒 同時上線 **`unlisted` frontmatter 欄位**：標 `unlisted: true` → 首頁列表／搜尋／計數皆跳過、網址仍可直接開；⚠️ unlisted ≠ private，真私密內容另走 `.gitignore`）
